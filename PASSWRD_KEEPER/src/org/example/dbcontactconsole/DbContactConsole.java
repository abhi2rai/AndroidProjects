...
public class DbContactConsole extends ListActivity {
	private static String[] FROM = { _ID, DbConstants.SUBJECT, DbConstants.USERNAME, DbConstants.PASSWORD, DbConstants.QUESTION, DbConstants.ANSWER, DbConstants.NOTES, };
	   private DbCreate contacts;
	   private static SQLiteDatabase db;
	   private static int[] TO = { R.id.rowid, R.id.name, R.id.mobilephone, R.id.email, R.id.ques, R.id.ans, R.id.notes };
		   ...
		   switch (item.getItemId()){
		       case R.id.modifyitem:
		    	   if(null != itemId){
		    		   ...
		    		   contactToModify.putString("cQues", cursor.getString(4));
		    		   contactToModify.putString("cAns", cursor.getString(5));
		    		   contactToModify.putString("cNotes", cursor.getString(6));
	 ...
	 //method removes item from database
	   private void removeContact(String itemId){
    	   ...
    	   db.delete(DbConstants.TABLE_NAME1, "_ID=" + itemId, null);
	   ...
	   private void addContact(Bundle bundle) {
		      ...
		      vals.put(DbConstants.SUBJECT, bundle.getString("contactFirstName"));
			  vals.put(DbConstants.USERNAME, bundle.getString("contactMobilePhone"));
			  vals.put(DbConstants.PASSWORD, bundle.getString("contactEmail"));
			  vals.put(DbConstants.QUESTION, bundle.getString("contactQuestion"));
			  vals.put(DbConstants.ANSWER, bundle.getString("contactAnswer"));
			  vals.put(DbConstants.NOTES, bundle.getString("contactNotes"));
			  db.insertOrThrow(DbConstants.TABLE_NAME1, null, vals);
		   }
	 ...
	 //method should modify existing Contact
	   private void modifyContact(Bundle bundle){
		   ...
		   vals.put(DbConstants.ANSWER, bundle.getString("contactAnswer"));
		   vals.put(DbConstants.NOTES, bundle.getString("contactNotes"));
		   db.update(DbConstants.TABLE_NAME1, vals, _ID + "=" + itemId, null);
	   }

	   private Cursor getContacts() {
		      db = contacts.getReadableDatabase();
		      cursor = db.query(DbConstants.TABLE_NAME1, FROM, null, null, null, null, null);
