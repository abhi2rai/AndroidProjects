...
public class ContactDetails extends Activity{
	...
	private static EditText quesField;
	private static EditText ansField;
	private static EditText notesField;
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		if (operationType.equals("addPerson")){
			...
			quesField =(EditText)findViewById(R.id.contactques);
			ansField = (EditText)findViewById(R.id.contactans);
			notesField = (EditText)findViewById(R.id.contactnotes);
			...
			String modifyQues =incomingActionType.getString("cQues");
			quesField=(EditText)findViewById(R.id.contactques);
			quesField.setText(modifyQues);
			
			String modifyAns = incomingActionType.getString("cAns");
			ansField=(EditText)findViewById(R.id.contactans);
			ansField.setText(modifyAns);
			
			String modifyNotes = incomingActionType.getString("cNotes");
			notesField =(EditText)findViewById(R.id.contactnotes);
			notesField.setText(modifyNotes);
		}
		...
		public void onClick(View v) {
			...
			Bundle contactData = new Bundle();
				...
				if (nameField.getText().toString().length() == 0){
					new AlertDialog.Builder(ContactDetails.this).setTitle("Error").setMessage("Subject field cannot be empty!").setNeutralButton("Close", null).show();
				} else if (mobilePhoneField.getText().toString().length() == 0){
					new AlertDialog.Builder(ContactDetails.this).setTitle("Error").setMessage("Username field cannot be empty!").setNeutralButton("Close", null).show();
				} else if (emailField.getText().toString().length() == 0){
					new AlertDialog.Builder(ContactDetails.this).setTitle("Error").setMessage("Password field cannot be empty!").setNeutralButton("Close", null).show();
				} else{
		            ...
		            contactData.putString("contactQuestion", quesField.getText().toString());
		            contactData.putString("contactAnswer", ansField.getText().toString());
		            contactData.putString("contactNotes",notesField.getText().toString());
